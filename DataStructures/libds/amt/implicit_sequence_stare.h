...
namespace ds::amt {
    ...
    {
		// hotovo 03
		return 0 < this->size() ? &this->getMemoryManager()->getBlockAt(0) : nullptr;
	}
	...
	typename ImplicitSequence<DataType>::BlockType* ImplicitSequence<DataType>::accessLast() const
    {
		...
		return 0 < size ? &this->getMemoryManager()->getBlockAt(size - 1) : nullptr;
	}
	...
	typename ImplicitSequence<DataType>::BlockType* ImplicitSequence<DataType>::accessNext(const BlockType& block) const
    {
		...
		return index < this->size() ? &memManager->getBlockAt(index)->getBlockAt(index) : nullptr;
    ...
    void ImplicitSequence<DataType>::removeNext(const BlockType& block)
	{
		// hotovo 03
		MemoryManagerType* memManager = this->getMemoryManager();
		memManager->releaseMomoryAt(indexOfNext(&memManager->calculateIndex(block)));
	}
    ...
    void ImplicitSequence<DataType>::removePrevious(const BlockType& block)
	{
		// hotovo 03
		MemoryManagerType* memManager = this->getMemoryManager();
		memManager->releaseMomoryAt(this->indexOfPrevious(&memManager->calculateIndex(block)));
	}
    ...
    size_t ImplicitSequence<DataType>::indexOfNext(size_t currentIndex) const
	{
		...
		//return currentIndex >= this->getMemoryManager->size() ? INVALID_INDEX : currentIndex + 1;
    ...
    size_t ImplicitSequence<DataType>::indexOfPrevious(size_t currentIndex) const
	{
		...
		/*if (currentIndex <= 0) 
		{
			throw std::runtime_error("NEPLATNY INDEX");
		}
		else 
		{
