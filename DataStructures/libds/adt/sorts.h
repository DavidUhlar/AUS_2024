...
namespace ds::adt
{
    ...
    {
        size_t pivotNumber = min + (max - min) / 2;
        T pivot = is.access(pivotNumber)->data_;
        size_t leftNumber = min;
        size_t rightNumber = max;

        
        do
        {
            while (compare(is.access(leftNumber)->data_, pivot))
            {
                ++leftNumber;
            }

            while (compare(pivot, is.access(rightNumber)->data_) && rightNumber > 0)
            {
                --rightNumber;
            }
            ...
            {
                std::swap(is.access(leftNumber)->data_, is.access(rightNumber)->data_);
                ++leftNumber;
                if (rightNumber > 0)
                {
                    --rightNumber;
        ...
        if (min < rightNumber)
        {
            this->quick(is, compare, min, rightNumber);
        }

        if (leftNumber < max)
        {
            this->quick(is, compare, leftNumber, max);
        }
    }
